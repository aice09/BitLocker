...
function Identify-DeviceType {
    try {
        ...
        $errorMessage = "Error identifying device type: $_"
        Log-Message $errorMessage
        Exit-Script -message $errorMessage
    }
}


# Get device type
$deviceType = Identify-DeviceType
# Get device info
$driveLetter = "C"
...
Log-Message "----- Device Type Verification -----"

# Check device type
if ($deviceType -ne "Laptop") {
    $deviceInfo = "Device Type: $deviceType`n"
    $deviceInfo += "BitLocker only allowed to install to laptop devices.`n"
    $deviceInfo += "Exiting script after 5 seconds."

    Exit-Script -message $deviceInfo
}
...
if ($bitlockerStatus -eq "FullyEncrypted" -and $bitlockerProtectionStatus -eq "On") {
    # When BitLocker is fully encrypted or protection is on
...
elseif ($bitlockerStatus -eq "FullyEncrypted" -and $bitlockerProtectionStatus -eq "Off") {
    ...
    # When BitLocker is not fully encrypted or protection is off
